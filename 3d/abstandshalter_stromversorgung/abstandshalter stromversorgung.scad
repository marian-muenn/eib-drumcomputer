...
module spacer(){
    difference(){
        cylinder(1, 3.5, 3.5, $fn=12);
        translate([0,0,-1])
        cylinder(14, 1.65, 1.65, $fn=12);
    }
}
module mount(){
    difference(){
        ...
        translate([3.2, 5,0]) rotate([0,0,30]) nutHole2(tolerance=0.15);
    }
}
module nutHole2(units=MM, tolerance = +0.05, proj = -1)
{
	//takes a metric screw/nut size and looksup nut dimensions
	radius = 5/2 /0.866+tolerance;
	height = 3+tolerance;
	if (proj == -1)
	{
